...
const upload = () => {
  ...
  const handleAnalyze = async ({ companyName, jobTitle, jobDescription, file }: { companyName: string, jobTitle: string, jobDescription: string, file: File  }) => {
    ...
    if(!uploadedFile) return alert('Failed to upload file. Please try again.');

    setStatusText('Generating feedback...');
    const imageFile = await convertPdfToImage(file);

    if (!imageFile.file) return alert('Failed to convert PDF to image. Please try again.');

    setStatusText('Saving data...');

    const uploadedImage = await fs.upload([imageFile.file]);

    if (!uploadedImage) return alert('Failed to upload image. Please try again.');
